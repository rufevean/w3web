...
async function deployContracts() {
  ...
  const ArxNFT_instance = await arxPropertyManager.getarxNFTAddress();
  ...
  const timelock = await Timelock.deploy(
    100,
    ['0x0aF6Da4A119EA2CD63017a549509AC750cdeC06E', '0x9F68bFb0be48089B99773b4c27A3559124B24D92'], // Array of proposers addresses
  ...
  const setSharesForSaleTx = await arxPropertyManager.putSharesForSale(latestTokenId, ethers.utils.parseEther('10'));
  ...
  console.log("Buying the shares for the latest token ID... from another account");
  
  arxToken = await ArxNFT.attach(arxTokenAddress);
  arxToken = arxToken.connect(signer1);

  let buyShrares = await arxToken.purchaseShare(100, {value: ethers.utils.parseEther("0.1")});
