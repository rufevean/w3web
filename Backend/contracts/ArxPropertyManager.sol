...
//-------------------------------------events-------------------------------------
    ...
    event PropertyListed(address indexed _to, string _uri, string _leagalOwner, string _propertyAddress,string _propertyType, string _propertySize, string _propertyDescripton, uint256 _totalSupply, string _name, string _symbol, uint256 _tokenId, address _arxTokenAddress);
...
//-------------------------------------constructor-------------------------------------
    ...
    constructor(address _arxNFTAddress) {
        arxNFTAddress = _arxNFTAddress; // Storing the ArxNFT deployed contract address
        arxNFT = ArxNFT(_arxNFTAddress); // ArxNFT contract instance using the entered address
    }

//-------------------------------------Internal Functions-------------------------------------

    function _mintPropertyNFT(address _to, string memory _uri, string memory _leagalOwner, string memory _propertyAddress,string memory _propertyType, string memory _propertySize, string memory _propertyDescripton) internal returns(uint256) {
        arxNFT.safeMint(_to, _uri, _leagalOwner, _propertyAddress, _propertyType, _propertySize, _propertyDescripton); // Minting the ArxNFT for the entered property details
        return arxNFT.getLatestTokenId(); // Returning the latest tokenId
    }

//-------------------------------------Public Functions-------------------------------------
    
    function returnArxTokenAddress(uint256 _tokenId) public view returns(address) {
        return arxTokenIdToArxTokenAddress[_tokenId]; // Returning the ArxToken contract address for the entered ArxNFT tokenId
    }

//-----------------------------OnlyOwner Functions-------------------------------------

    function listProperty(address _to, string memory _uri, string memory _leagalOwner, string memory _propertyAddress,string memory _propertyType, string memory _propertySize, string memory _propertyDescripton, uint256 _totalSupply, string memory _name, string memory _symbol) public onlyOwner {
        uint256 tokenId = _mintPropertyNFT(_to, _uri, _leagalOwner, _propertyAddress, _propertyType, _propertySize, _propertyDescripton); // Minting the ArxNFT for the entered property details
        ...
        arxToken.initialize(address(arxNFT), tokenId); // Initializing the ArxToken contract

        emit PropertyListed(_to, _uri, _leagalOwner, _propertyAddress, _propertyType, _propertySize, _propertyDescripton, _totalSupply, _name, _symbol, tokenId, address(arxToken)); // Emitting the PropertyListed event
    }
    /*-------------------------------------Governance Functions-------------------------------------*/
    function putSharesForSale(uint256 _tokenId, uint256 _price) onlyOwner public {
        ArxToken arxTokenInstatk = ArxToken(payable(arxTokenIdToArxTokenAddress[_tokenId])); // ArxToken contract instance using the ArxToken contract address
    ...
    function putNFTForSale(uint256 _tokenId, uint256 _price) onlyOwner public{
        ArxToken arxTokenInstatk = ArxToken(payable(arxTokenIdToArxTokenAddress[_tokenId])); // ArxToken contract instance using the ArxToken contract address
